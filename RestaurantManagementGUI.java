...
class Dish {
        ...
        public String placeOrder(String dishName, int quantity) {
            for (Dish dish : menu) {
                if (dish.getName().equalsIgnoreCase(dishName)) { 
                    // So sánh tên món ăn (không phân biệt chữ hoa/thường)
                    orders.add(new Order(dishName, quantity));
                    return "Đặt món thành công!";
            }
        }
            return "Món không có trong thực đơn!"; 
            // Trả về thông báo nếu món không có trong thực đơn
    }
            ...
            for (Order order : orders) {
                for (Dish dish : menu) {
                    if (order.getDishName().equals(dish.getName())) {
                        totalRevenue += dish.getPrice() * order.getQuantity(); 
                        // Tính doanh thu cho từng món ăn đã đặt
                }
            }
        }
        return totalRevenue;
    ...
    public double calculateTotalAmountForOrder() {
        ...
        for (Order order : orders) {
            for (Dish dish : menu) {
                if (order.getDishName().equals(dish.getName())) {
                    totalAmount += dish.getPrice() * order.getQuantity(); 
                    // Tính tổng tiền cho các món đã đặt
                }
            }
        }
        return totalAmount;
...
// Lớp chính để tạo giao diện người dùng cho quản lý nhà hàng
   public class RestaurantManagementGUI {
    ...
    private void createAndShowGUI() {
        ...
        // Tạo tabbed pane để chia giao diện thành các tab

        // Tab quản lý thực đơn
        JPanel menuPanel = new JPanel();
            menuPanel.setLayout(new BorderLayout()); // Đặt bố cục cho panel
            menuPanel.add(createMenuButtons(), BorderLayout.NORTH); 
            // Thêm nút vào phía trên
            menuPanel.add(createMenuTablePanel(), BorderLayout.CENTER); 
            // Thêm bảng vào giữa
        ...
        JPanel statisticsPanel = new JPanel();
            ...
            tabbedPane.addTab("Thống Kê & Doanh Thu", statisticsPanel); 
            // Thêm tab Thống Kê & Doanh Thu
    ...
    public static void main(String[] args) {
        new RestaurantManagementGUI(); 
        // Khởi tạo đối tượng GUI và hiển thị giao diện
    }
}
